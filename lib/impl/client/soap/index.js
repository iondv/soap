    ...
    return new Promise((resolve, reject) => {
      ...
      request.get(
        ...
        function (err, response, body) {
          if (err) {
            if (options.log) {
              options.log.warn('Не удалось получить xml-схему по адресу ' + schemaUrl + '.');
            }
            return resolve([]);
          }
          if (response.statusCode === 200) {
            ...
            if (options.log) {
              options.log.warn('Не удалось получить xml-схему по адресу ' +
                schemaUrl + '. Код ответа ' + response.statusCode);
            }
            resolve([]);
          }
        }
                    ...
                    try {
                      ...
                      for (let i = 0; i < services.length; i++) {
                        ...
                        if (portType.length > 0) {
                          ...
                          if (options.log) {
                            options.log.info('Инициализирован клиент сервиса ' + service.getAttribute('name') + ' для адреса ' + wsdl);
                          }
                        } else {
                          return reject('WSDL не содержит описания интерфейсов.');
                        }
                      }
                      return resolve(meta);
                    } catch (err) {
      ...
      .catch((err) => {
        if (options.interval) {
          if (options.log) {
            ...
            options.log.warn(`Не удалось подключиться по адресу ${wsdl} [${err.message}]`);
          }
          clientEvents.emit('failConnect', descriptor, mapping);
        } else {
          throw err;
  ...
  if (options.interval) {
    ...
    clientEvents.on('failConnect', (descriptor, mapping) => {
      ...
      if (attempt) {
        setTimeout(() => {
          if (options.log) {
            options.log.info(`Повторная загрузка (${attempts[wsdl]}) клиента сервиса для адреса ${wsdl}`);
          }

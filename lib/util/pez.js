...
internals.contentDisposition = function (header) {
  ...
  const result = {};
  result.dispositionType = match[1];
...
internals.Dispenser.prototype._onLineEnd = function () {
  ...
  if (this._state === internals.state.header) {
    ...
    try {
      disposition = internals.contentDisposition(this._headers['content-disposition']);
    }
    catch (err) {
      // return this._abort(err);
    }
    ...
    stream.contentId = this._headers['content-id'];
    stream.isAttachment = disposition && disposition.dispositionType === 'attachment' ? true : false;
    stream.name = disposition && disposition.name || stream.contentId;
    stream.filename = disposition && disposition.filename || stream.contentId;
